...
export class RuleBasedProvider extends Provider {
  ...
  async generate({message="",intent="conversation",pragmatics={},state={},memory=[]}={}) {
    ...
    const turn=(state.turns||0)+this.seed;
    let response;

    if(intent==="greeting") {
      response=["Oi! Pode falar, estou acompanhando.","Opa! Manda, o que você queria me contar?","Oi! Tô aqui. Pode continuar."][turn%3];
      ...
      response=["Fechou. Pode continuar quando quiser.","Por nada! Seguimos.","Certo. Tô acompanhando."][turn%3];
      ...
      response=["Entendi. Continua, tô acompanhando.","Certo — me conta mais dessa parte.","Ah, entendi a direção. O que aconteceu depois?","Tá. Qual é o ponto que você quer explorar primeiro?"][turn%4];
    }
